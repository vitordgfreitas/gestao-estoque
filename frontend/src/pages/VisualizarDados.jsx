...
import { Search, Edit, Trash2, Eye, Package, Calendar, Car, DollarSign, MapPin, Hash, Coins, ClipboardList, PenTool } from 'lucide-react'
...
export default function VisualizarDados() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      // O segredo está aqui: Sua API precisa retornar compromisso_itens(quantidade, itens(nome))
  ...
  const stats = useMemo(() => {
    // Patrimônio Total = soma de (valor_compra * quantidade_total) de todos os itens
    const patrimonio = itens.reduce((acc, i) => acc + (Number(i.valor_compra) * i.quantidade_total || 0), 0)
    // Receita = soma da coluna valor_total_contrato da tabela compromissos
  ...
  const filteredData = useMemo(() => {
    ...
    return {
      ...
      compromissos: compromissos.filter(c => 
        c.contratante?.toLowerCase().includes(term) || 
        c.nome_contrato?.toLowerCase().includes(term)
      )
    }
  }, [searchTerm, itens, compromissos])

  // --- HANDLERS ---
  const handleSaveItem = async (data) => {
    try {
      await itensAPI.atualizar(editingItem.id, data)
      toast.success('Ativo e dados_categoria salvos!')
  ...
  const handleSaveCompromisso = async (data) => {
    try {
      ...
      toast.success('Faturamento e dados do contrato atualizados!')
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto px-4 pb-10">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-end">
        <div>
          ...
          <p className="text-dark-400 font-medium tracking-widest">Star Gestão • Operação Brasília, DF</p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <StatMini label="Manutenções" value={stats.totalManutencao} color="text-blue-400" />
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="input w-full pl-14 h-16 text-xl border-dark-700 bg-dark-800/60 backdrop-blur-md rounded-2xl" 
        />
      </div>
      ...
      <div className="card overflow-hidden p-0 border-dark-700 bg-dark-900/40 backdrop-blur-lg shadow-2xl">
        <div className="overflow-x-auto">
          ...
          {/* ABA ITENS: Mostra Patrimônio e Data Aquisição */}
          {activeTab === 'itens' && (
              ...
              <tbody className="divide-y divide-dark-800">
                {filteredData.itens.map(item => (
                  <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-6 py-5 text-green-400 font-black font-mono text-lg">
                      R$ {(item.valor_compra * item.quantidade_total || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                    </td>
          ...
          {/* ABA CONTRATOS: Mostra Receita e Itens Vinculados */}
          {activeTab === 'compromissos' && (
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead className="bg-dark-800/50 text-dark-400 text-[11px] uppercase font-black tracking-widest">
                <tr>
                  <th className="px-6 py-4 w-[25%]">Contrato / Cliente</th>
                  <th className="px-6 py-4 w-[35%] text-center">Equipamentos Alugados</th>
                ...
                {filteredData.compromissos.map(c => (
                  <tr key={c.id} className="hover:bg-green-500/[0.02] transition-colors group">
                    <td className="px-6 py-5">
                      ...
                      <div className="text-[10px] text-dark-500 font-bold uppercase mt-1 tracking-tighter">{c.contratante}</div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-[11px] font-medium">
                       ...
                       <div className="text-dark-500 mt-1 italic uppercase">até {formatDate(c.data_fim)}</div>
                    </td>
          ...
          {/* ABA MANUTENÇÃO (PEÇAS_CARROS) */}
          {activeTab === 'pecas-carros' && (
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead className="bg-dark-800/50 text-dark-400 text-[11px] uppercase font-black tracking-widest">
                <tr>
                  <th className="px-6 py-4 w-[30%]">Veículo Alvo</th>
                  <th className="px-6 py-4 w-[30%]">Peça / Componente</th>
                ...
                {pecasCarros.map(pc => {
                  ...
                  return (
                    <tr key={pc.id} className="hover:bg-blue-500/[0.02] transition-colors group">
                      ...
                      <td className="px-6 py-5 font-black text-lg text-center">{pc.quantidade}</td>
                      <td className="px-6 py-5 text-xs text-dark-400 font-mono">{formatDate(pc.data_instalacao)}</td>
...
// --- MODAIS DE EDIÇÃO (O QUE SALVA O DIA) ---
      ...
      <form onSubmit={(e) => {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="label">Qtd Estoque</label><input type="number" className="input" value={formData.quantidade_total} onChange={e => setFormData({...formData, quantidade_total: e.target.value})} /></div>
            <div><label className="label text-green-500">Custo Unitário</label><input className="input" value={formData.valor_compra} onChange={e => setFormData({...formData, valor_compra: e.target.value})} /></div>
            <div className="md:col-span-2"><label className="label">Data Aquisição</label><input type="date" className="input" value={formData.data_aquisicao} onChange={e => setFormData({...formData, data_aquisicao: e.target.value})} /></div>
          </div>
        </div>
        ...
        <button type="submit" className="btn btn-primary w-full py-4 font-black">SALVAR ALTERAÇÕES NO PATRIMÔNIO</button>
      </form>
    </Modal>
  )
}
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Editar Contrato Master">
      <form onSubmit={(e) => {
        ...
        <div><label className="label">Nome do Contrato / Evento</label><input className="input font-bold" value={formData.nome_contrato} onChange={e => setFormData({...formData, nome_contrato: e.target.value})} /></div>
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Editar Manutenção">
      <div className="mb-4 p-4 bg-dark-800 rounded-2xl border border-dark-700 text-xs shadow-inner">
        ...
        <p className="flex justify-between mt-1 text-blue-400 font-black uppercase tracking-tighter"><strong>Peça Instalada:</strong> <span>{p?.nome}</span></p>
      </div>
      <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="space-y-4">
        ...
        <button type="submit" className="btn btn-primary w-full py-3 font-bold">ATUALIZAR REGISTRO</button>
      </form>
    </Modal>
  )
}
      ...
      <div className="space-y-6">
        ...
        {item.dados_categoria && (
          <div className="p-6 bg-dark-800/50 rounded-3xl border border-dark-700 shadow-2xl">
            <h4 className="text-[10px] font-black text-primary-500 uppercase tracking-widest mb-4 border-b border-dark-700 pb-2">Detalhes da Categoria</h4>
