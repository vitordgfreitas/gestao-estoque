...
export default function ContasReceber() {
  ...
  const loadCompromissos = async () => {
    try {
      const response = await compromissosAPI.listar()
  ...
  const loadContas = async () => {
    try {
      ...
      const response = await contasReceberAPI.listar(params)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      await contasReceberAPI.criar({
        ...formData,
        valor: parseFloat(formData.valor),
        compromisso_id: parseInt(formData.compromisso_id)
      })
  ...
  const handleMarcarPaga = async (id) => {
    try {
      await contasReceberAPI.marcarPaga(id, { data_pagamento: new Date().toISOString().split('T')[0] })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-dark-50">Contas a Receber</h1>
        <button
      ...
      {showForm && (
        ...
        >
          <h2 className="text-xl font-semibold text-dark-50 mb-4">Nova Conta a Receber</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-2">Compromisso *</label>
                <select
                  required
                  value={formData.compromisso_id}
                  onChange={(e) => {
                    const comp = compromissos.find(c => c.id === parseInt(e.target.value))
                    setFormData({
                      ...formData,
                      compromisso_id: e.target.value,
                      descricao: comp ? `Aluguel - ${comp.descricao || 'Sem descrição'}` : formData.descricao
                    })
                  }}
                ...
                >
                  ...
                  {compromissos.map(comp => (
                    <option key={comp.id} value={comp.id}>
                      {comp.descricao || `Compromisso #${comp.id}`} - {comp.data_inicio} a {comp.data_fim}
                    </option>
                  ))}
                </select>
              </div>
      ...
      {loading && contas.length === 0 ? (
        ...
        <div className="text-center py-12 text-dark-400">Nenhuma conta a receber encontrada</div>
      ) : (
            ...
            <table className="w-full">
              <thead className="bg-dark-700/50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase">Compromisso</th>
              ...
              <tbody className="divide-y divide-dark-700">
                {contasFiltradas.map((conta) => (
                  <tr key={conta.id} className="hover:bg-dark-700/30">
                    ...
                    <td className="px-6 py-4 text-sm text-dark-200">#{conta.compromisso_id}</td>
