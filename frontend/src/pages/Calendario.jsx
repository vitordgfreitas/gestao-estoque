...
export default function Calendario() {
  ...
  // Sincroniza Financeiro (Boletos) do mês visível em Brasília
  useEffect(() => {
    let active = true
  ...
  const loadData = async () => {
    try {
      ...
      toast.error('Erro ao conectar com o servidor da Star Gestão.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto px-4 pb-10">
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-end">
        <div>
          ...
          <p className="text-dark-400 font-medium tracking-widest uppercase text-[10px]">Logística e Vencimentos • Brasília, DF</p>
        </div>
      ...
      {/* 4. MODAL DE DETALHES (image_85d57b.png Fix) */}
      {detalhesDia && (
        <Modal isOpen={true} onClose={() => setDetalhesDia(null)} title={`Agenda: ${formatDate(dataToStr(detalhesDia.data))}`}>
          <div className="space-y-8 max-h-[70vh] overflow-y-auto pr-3 custom-scrollbar">
            ...
            <Section title="Financeiro e Boletos" icon={<DollarSign size={14}/>} color="text-green-500">
              ...
              ) : <p className="text-dark-600 text-xs italic text-center py-4">Sem vencimentos para hoje.</p>}
            </Section>
          </div>
        </Modal>
      )}
...
// --- COMPONENTES ATÔMICOS (DECLARADOS LOCALMENTE PARA EVITAR TELA BRANCA) ---
    ...
    days.push(
      <button key={d} onClick={() => onDayClick(data)} className={`relative p-2 border border-dark-800/40 hover:bg-primary-500/5 transition-all text-left flex flex-col h-full min-h-[110px] overflow-hidden ${isHoje ? 'bg-primary-500/10' : ''}`}>
        ...
        <div className="space-y-1 w-full">
          {ev.iniciam.slice(0, 1).map(c => <div key={c.id} className="text-[7px] bg-primary-600 text-white font-black px-1.5 py-0.5 rounded truncate uppercase">🚀 {c.nome_contrato || 'Contrato'}</div>)}
...
function DailyView({ data, eventos, setDia, abrirDetalhes }) {
  const move = (dir) => {
    const next = new Date(data.getTime()) // Navegação imutável
  ...
  return (
    <div className={`p-8 rounded-[2rem] border shadow-2xl ${isPrimary ? 'bg-primary-500/[0.03] border-primary-500/20' : 'bg-green-500/[0.03] border-green-500/20'}`}>
      ...
      <div className="space-y-3">
        ...
        {(!items || items.length === 0) && <p className="text-dark-600 text-[10px] uppercase font-bold italic opacity-50 text-center py-6">Vazio.</p>}
      </div>
    </div>
  )
}