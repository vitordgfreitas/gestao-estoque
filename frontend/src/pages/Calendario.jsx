...
// Importando SUAS funções de formatação
...
export default function Calendario() {
  ...
  // 🔥 SOLUÇÃO PARA O FUSO: Compara apenas STRINGS puras vindas do banco
  const dataToStr = (d) => {
    if (!d) return ''
  ...
  const handleSalvarBaixa = async (formData) => {
    ...
    try {
      ...
      toast.success('Pago com sucesso!', { id: loadId })
      setParcelaEmBaixa(null)
      loadData()
    } catch (e) { toast.error('Erro na baixa.', { id: loadId }) }
  }
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto px-4 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-2 flex items-center justify-between bg-dark-800 px-6 rounded-xl border border-dark-700 shadow-lg">
          <button onClick={() => setMesAtual(prev => prev === 1 ? 12 : prev - 1)} className="p-2 text-primary-400"><ChevronLeft/></button>
          <span className="text-lg font-black text-dark-50 uppercase">{new Date(anoAtual, mesAtual-1).toLocaleString('pt-BR', { month: 'long', year: 'numeric' })}</span>
          <button onClick={() => setMesAtual(prev => prev === 12 ? 1 : prev + 1)} className="p-2 text-primary-400"><ChevronRight/></button>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden border-dark-700 bg-dark-900/40 shadow-2xl">
        <div className="grid grid-cols-7 border-b border-dark-700 bg-dark-800/80">
          {['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map(d => <div key={d} className="py-4 text-center text-[10px] font-black text-dark-500 uppercase tracking-widest">{d}</div>)}
        </div>
                       ...
                       {detalhesDia.parcelas.map(p => (
                         <tr key={p.id} className="hover:bg-green-500/5 transition-colors">
                           ...
                           <td className="p-5 text-center">
                             <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full font-black text-[8px] border ${p.status === 'Paga' ? 'text-green-400 border-green-500/20 bg-green-500/5' : 'text-yellow-400 border-yellow-500/20 bg-yellow-500/5'}`}>
                               {p.status}
                             </span>
                           </td>
                           <td className="p-5">
                              <div className="flex justify-center gap-2">
                                {p.link_boleto && <a href={p.link_boleto} target="_blank" rel="noreferrer" className="p-2 bg-dark-700 text-primary-400 rounded-xl"><FileText size={18}/></a>}
                                {p.status !== 'Paga' && <button onClick={() => setParcelaEmBaixa(p)} className="p-2 bg-green-500 text-white rounded-xl shadow-lg"><Check size={18}/></button>}
                                {p.link_comprovante && <a href={p.link_comprovante} target="_blank" rel="noreferrer" className="p-2 bg-dark-700 text-green-400 rounded-xl"><Receipt size={18}/></a>}
                              </div>
                           </td>
                         </tr>
                       ))}
      ...
      {parcelaEmBaixa && (
        <Modal isOpen={true} onClose={() => setParcelaEmBaixa(null)} title="Baixa de Pagamento">
           <form onSubmit={(e) => {
             e.preventDefault();
             handleSalvarBaixa(Object.fromEntries(new FormData(e.target)));
           }} className="space-y-6">
              ...
              <div><label className="label uppercase text-[10px] font-black">Valor Pago</label><input name="valor_pago" type="number" step="0.01" className="input font-mono text-green-400" defaultValue={parcelaEmBaixa.valor_original} required /></div>
              <div><label className="label uppercase text-[10px] font-black">Link Comprovante</label><input name="link_comprovante" type="url" placeholder="https://..." className="input" /></div>
              <button type="submit" className="btn btn-primary w-full py-4 font-black uppercase shadow-xl shadow-primary-500/40">Confirmar Baixa</button>
           </form>
        </Modal>
      )}
    </div>
...
function ViewBtn({ active, icon, label, onClick }) {
  return <button onClick={onClick} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-xs font-black transition-all ${active ? 'bg-primary-500 text-white shadow-xl' : 'text-dark-400 hover:text-dark-100 hover:bg-dark-700'}`}>{icon} {label}</button>
}
function FilterSelect({ icon, value, onChange, options }) {
  return <div className="relative flex-1 group"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-dark-500">{icon}</div><select value={value} onChange={e => onChange(e.target.value)} className="input pl-12 h-14 bg-dark-800/50 border-dark-700 rounded-2xl">{(options || []).map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div>
}
function Section({ title, icon, color, children }) {
  return <div className="space-y-5"><h4 className={`text-[11px] font-black ${color} uppercase tracking-[0.3em] flex items-center gap-3 border-b border-dark-700 pb-3`}>{icon} {title}</h4>{children}</div>
}
function InfoBox({ icon, label, value }) {
  return <div className="bg-dark-900/40 p-4 rounded-2xl border border-dark-700/50 h-full"><p className="text-[9px] font-black text-dark-500 uppercase mb-2 flex items-center gap-2">{icon} {label}</p><p className="text-xs text-dark-200 leading-relaxed font-medium">{value}</p></div>
}
function EmptyState({ icon, text }) {
  return <div className="py-20 text-center bg-dark-800/30 rounded-[3rem] border border-dashed border-dark-700"><div className="text-dark-600 opacity-20 mb-4 flex justify-center">{icon}</div><p className="text-dark-500 text-[11px] font-bold uppercase tracking-widest italic">{text}</p></div>
}
function renderCalendarDays(ano, mes, getEventos, onDayClick) {
  ...
  for (let d = 1; d <= end.getDate(); d++) {
    ...
    days.push(<button key={d} onClick={() => onDayClick(data)} className={`relative p-3 border border-dark-800/40 hover:bg-primary-500/5 text-left flex flex-col h-full min-h-[120px] overflow-hidden ${isHoje ? 'bg-primary-500/10 ring-1 ring-primary-500/30 shadow-[0_0_15px_rgba(139,92,246,0.2)]' : ''}`}><span className={`text-xs font-black mb-2 ${isHoje ? 'text-primary-400 underline underline-offset-4' : 'text-dark-500'}`}>{d < 10 ? `0${d}` : d}</span><div className="space-y-1.5 w-full">{ev.iniciam.slice(0, 1).map(c => <div key={c.id} className="text-[7px] bg-primary-600 text-white font-black px-2 py-0.5 rounded truncate uppercase shadow-lg">🚀 {c.nome_contrato}</div>)}{ev.ativos.length > 0 && <div className="text-[7px] text-dark-300 font-bold px-1.5 py-0.5 border border-dark-700 rounded bg-dark-800/90 truncate flex items-center gap-1"><Package size={8}/> {ev.ativos.length} Ativos</div>}{ev.parcelas.length > 0 && <div className="text-[7px] bg-green-500/20 text-green-500 font-black px-1.5 py-0.5 rounded border border-green-500/20 truncate flex items-center gap-1 shadow-inner">💰 {ev.parcelas.length} Venc.</div>}</div></button>)
  }
  return days
}
function DailyView({ data, eventos, setDia, abrirDetalhes }) {
  const move = (dir) => { const next = new Date(data.getTime()); next.setDate(next.getDate() + (dir === 'ant' ? -1 : 1)); setDia(next) }
  return <div className="space-y-8"><div className="flex items-center justify-between bg-dark-800/80 p-12 rounded-[3rem] border border-dark-700 shadow-2xl backdrop-blur-2xl"><button onClick={() => move('ant')} className="p-5 hover:bg-dark-700 rounded-[2rem] text-primary-400 active:scale-90 transition-all"><ChevronLeft size={40}/></button><div className="text-center"><h2 className="text-6xl font-black text-dark-50 uppercase tracking-tighter mb-2 italic">{data.toLocaleDateString('pt-BR', { weekday: 'long' })}</h2><p className="text-primary-500 font-black tracking-[0.5em] uppercase text-base">{data.toLocaleDateString('pt-BR')}</p></div><button onClick={() => move('prox')} className="p-5 hover:bg-dark-700 rounded-[2rem] text-primary-400 active:scale-90 transition-all"><ChevronRight size={40}/></button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10"><DayCard title="Logística" icon={<PlayCircle size={24}/>} count={eventos.iniciam.length} color="primary" items={eventos.iniciam} onClick={() => abrirDetalhes(data)} /><DayCard title="Financeiro" icon={<DollarSign size={24}/>} count={eventos.parcelas.length} color="green" items={eventos.parcelas} onClick={() => abrirDetalhes(data)} /></div></div>
}
function DayCard({ title, icon, count, color, items, onClick }) {
  const isPrimary = color === 'primary'; return <div className={`p-10 rounded-[3rem] border shadow-2xl ${isPrimary ? 'bg-primary-500/[0.04] border-primary-500/20' : 'bg-green-500/[0.04] border-green-500/20'}`}><div className="flex justify-between items-center mb-10"><h3 className={`text-[12px] font-black uppercase tracking-[0.4em] flex items-center gap-4 ${isPrimary ? 'text-primary-500' : 'text-green-500'}`}>{icon} {title}</h3><span className={`text-xs font-black px-4 py-1.5 rounded-full text-white ${isPrimary ? 'bg-primary-500' : 'bg-green-500'} shadow-xl`}>{count}</span></div><div className="space-y-4">{(items || []).slice(0, 6).map(i => (<div key={i.id} onClick={onClick} className="p-6 bg-dark-800 rounded-[2rem] border border-dark-700 flex justify-between items-center cursor-pointer hover:border-primary-500/40 transition-all group shadow-lg"><div><p className="font-black text-dark-50 text-base truncate">{i.nome_contrato || i.codigo_contrato || 'Operação'}</p>{i.contratante && <p className="text-[11px] text-dark-500 font-bold uppercase mt-1 tracking-widest italic">{i.contratante}</p>}{i.valor_original && <p className="text-green-400 font-mono text-xs font-black mt-2">{formatCurrency(i.valor_original)}</p>}</div><div className="p-3 bg-dark-900 rounded-2xl group-hover:bg-primary-500 transition-colors"><ArrowRight size={20} className="text-dark-400 group-hover:text-white"/></div></div>))}</div></div>
}